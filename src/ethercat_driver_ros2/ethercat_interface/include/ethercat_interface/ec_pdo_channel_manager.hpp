...
public:
  ...
  void set_command_multiplier(double multiplier)
  {
    command_multiplier_ = multiplier;
  }

  void set_state_multiplier(double multiplier)
  {
    state_multiplier_ = multiplier;
  }
  ...
  void ec_update(uint8_t * domain_address)
  {
    ...
    if (pdo_type == TPDO) {
      ...
      if (interface_index >= 0) {
        state_interface_ptr_->at(interface_index) = state_multiplier_ * last_value;
      }
        ...
        !override_command)
      {
        ec_write(
          domain_address,
          factor * command_multiplier_ * command_interface_ptr_->at(interface_index) + offset);
      } else {
  ...
  double factor = 1;
  double offset = 0;
  double command_multiplier_ = 1;
  double state_multiplier_ = 1;
