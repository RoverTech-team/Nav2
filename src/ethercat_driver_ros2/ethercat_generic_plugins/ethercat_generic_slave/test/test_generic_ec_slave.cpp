...
TEST_F(GenericEcSlaveTest, EcWriteRPDOVelocityCommandInvertedForRightSideJoint)
{
  SetUp();
  std::unordered_map<std::string, std::string> slave_paramters;
  std::vector<double> command_interface = {42};
  plugin_->command_interface_ptr_ = &command_interface;
  slave_paramters["joint_name"] = "base_front_right_wheel_joint";
  slave_paramters["command_interface/velocity"] = "0";
  plugin_->paramters_ = slave_paramters;
  plugin_->setup_from_config(YAML::Load(test_slave_config));
  plugin_->setup_interface_mapping();
  ASSERT_EQ(plugin_->pdo_channels_info_[1].interface_index, 0);
  ASSERT_EQ(plugin_->pdo_channels_info_[1].command_multiplier_, -1);
  uint8_t domain_address[4];
  plugin_->processData(1, domain_address);
  ASSERT_EQ(plugin_->pdo_channels_info_[1].last_value, -42);
  ASSERT_EQ(EC_READ_S32(domain_address), -42);
}

TEST_F(GenericEcSlaveTest, EcWriteRPDOVelocityCommandUnchangedForLeftSideJoint)
{
  SetUp();
  std::unordered_map<std::string, std::string> slave_paramters;
  std::vector<double> command_interface = {42};
  plugin_->command_interface_ptr_ = &command_interface;
  slave_paramters["joint_name"] = "base_front_left_wheel_joint";
  slave_paramters["command_interface/velocity"] = "0";
  plugin_->paramters_ = slave_paramters;
  plugin_->setup_from_config(YAML::Load(test_slave_config));
  plugin_->setup_interface_mapping();
...
TEST_F(GenericEcSlaveTest, EcReadTPDOVelocityInvertedForRightSideJoint)
{
  SetUp();
  std::unordered_map<std::string, std::string> slave_paramters;
  std::vector<double> state_interface = {0};
  plugin_->state_interface_ptr_ = &state_interface;
  slave_paramters["joint_name"] = "base_front_right_wheel_joint";
  slave_paramters["state_interface/velocity"] = "0";
  plugin_->paramters_ = slave_paramters;
  plugin_->setup_from_config(YAML::Load(test_slave_config));
  plugin_->setup_interface_mapping();
  ASSERT_EQ(plugin_->pdo_channels_info_[7].interface_index, 0);
  ASSERT_EQ(plugin_->pdo_channels_info_[7].state_multiplier_, -1);
  uint8_t domain_address[4];
  EC_WRITE_S32(domain_address, 42);
  plugin_->processData(7, domain_address);
  ASSERT_EQ(plugin_->pdo_channels_info_[7].last_value, 42);
  ASSERT_EQ(plugin_->state_interface_ptr_->at(0), -42);
}

TEST_F(GenericEcSlaveTest, EcReadTPDOVelocityUnchangedForLeftSideJoint)
{
  SetUp();
  std::unordered_map<std::string, std::string> slave_paramters;
  std::vector<double> state_interface = {0};
