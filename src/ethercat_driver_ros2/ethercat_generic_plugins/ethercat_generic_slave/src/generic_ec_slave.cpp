...
namespace
{
bool is_right_side_wheel_joint(const std::string & joint_name)
{
  static const std::array<std::string, 3> kRightSideWheelJoints = {
  ...
  return std::find(
    kRightSideWheelJoints.begin(),
    kRightSideWheelJoints.end(),
    joint_name) != kRightSideWheelJoints.end();
}
}  // namespace
...
{
  const auto joint_name_it = paramters_.find("joint_name");
  const bool invert_velocity =
    joint_name_it != paramters_.end() && is_right_side_wheel_joint(joint_name_it->second);
  ...
  for (auto & channel : pdo_channels_info_) {
    ...
    if (invert_velocity && channel.interface_name == "velocity") {
      if (channel.pdo_type == ethercat_interface::RPDO) {
        channel.set_command_multiplier(-1.0);
      } else if (channel.pdo_type == ethercat_interface::TPDO) {
        channel.set_state_multiplier(-1.0);
      }
    }
  }
}
